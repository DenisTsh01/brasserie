...
if (process.env.NODE_ENV === 'development') {
  ...
  async function getBieresPression() {
    let response = await fetch(`${serverUrl}/api/bieres_pression/`);
    let data2 = await response.json();    
    console.log(data2)
    setDrinks(data2); 
  }
  async function getBieresBouteiless() {
    let response = await fetch(`${serverUrl}/api/bieres_bouteiless/`);
    let data2 = await response.json();    
    console.log(data2)
    setBieresBouteiless(data2); 
  }
  async function getSodas() {
    let response = await fetch(`${serverUrl}/api/sodas/`);
    let data2 = await response.json();    
    console.log(data2)
    setSodas(data2); 
  }
  ...
  async function getAlcools() {
    let response = await fetch(`${serverUrl}/api/alcools/`);
    let data2 = await response.json();    
    console.log(data2)
    setAlcools(data2); 
  }
  async function getAperities() {
    let response = await fetch(`${serverUrl}/api/aperities/`);
    let data2 = await response.json();    
    console.log(data2)
    setAperities(data2); 
  }
  async function getVins() {
    let response = await fetch(`${serverUrl}/api/vins/`);
    let data2 = await response.json();    
    console.log(data2)
    setVins(data2); 
  }
  async function getBoissonsChaudes() {
    let response = await fetch(`${serverUrl}/api/boissons_chaudes/`);
    let data2 = await response.json();    
    console.log(data2)
    setBoissonsChaudes(data2); 
  }
  async function getLiqueurs() {
    let response = await fetch(`${serverUrl}/api/liqueurs/`);
    let data2 = await response.json();    
    console.log(data2)
    setLiqueus(data2); 
  }
  ...
  async function getDessert() {
    let response = await fetch(`${serverUrl}/api/dessert/`);
    let data = await response.json();
    setDesserts(data);
  }

  async function getEntree() {
    let response = await fetch(`${serverUrl}/api/entree/`);
    let data = await response.json();
    setEntree(data);
  ...
  async function getPates() {
    let response = await fetch(`${serverUrl}/api/pates/`);
    let data = await response.json();
    setPates(data);
  }

  async function getPlat() {
    let response = await fetch(`${serverUrl}/api/plat/`);
    let data = await response.json();
    setPlat(data);
  }
  ...
  <div>
    ...
    {selectedMenu === 'Meal' ? (
     ...
     <div className="menu">
       ...
       <h2><b>Menu {menuTitle}</b> <br/><span className='hour'>11:30 - 14:30</span></h2>
